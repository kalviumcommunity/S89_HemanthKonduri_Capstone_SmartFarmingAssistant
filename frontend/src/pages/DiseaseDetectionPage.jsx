...
// Import the Navbar and new styles
import NavBar from '../components/NavBar';
import '../styles/DiseaseDetectionPage.css'; 
import { FaUpload, FaLeaf, FaImage, FaStore } from 'react-icons/fa';

// --- (The diseaseMapping object remains unchanged) ---
...
const DiseaseDetectionPage = () => {
    // --- (All your existing state and logic functions remain unchanged) ---
    ...
    const handleScanImage = async () => {
        ...
        try {
            ...
            let topPredictionText = predictions.map(p => p.className.split(',')[0]).join(', ');
    ...
    return (
        // A fragment is used to group the Navbar and the main content
        <>
            <NavBar />
                    ...
                    {previewUrl && !result && !loadingText && (
                        <div className="image-preview-section">
                            ...
                            <div className="preview-actions">
                                <button onClick={handleScanImage} className="btn btn-primary" disabled={!model}>
                                    {!model ? 'Model Loading...' : 'Scan Image'}
                                </button>
                                <button onClick={resetState} className="btn btn-secondary">Change Image</button>
                            </div>
                        </div>
                    )}
                    ...
                    {result && (
                        <div className="result-container">
                                        ...
                                        {result.recommendedProducts.map(prod => (
                                            <div key={prod._id} className="product-card-recommend">
                                                ...
                                                <div className="product-info-recommend">
                                                    <h5>{prod.name}</h5>
                                                    <Link to="/store" className="btn btn-tertiary">View in Store</Link>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="result-footer">
                                <button onClick={resetState} className="btn btn-secondary">Scan Another Image</button>
                            </div>
                        </div>
                    )}
                </div>
